...
Конструктор:  
`constructor(payment: 'card' | 'cash' | '', adress: string, phone: string, email: string)` - В конструктор передаются данные пользователя: тип оплаты, електронная почта, телефон и адресс.
