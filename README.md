...
#### Класс Gallery
    Определяет работу с компонентом интерфейса Gallery. Наследует от `Component<T>`.  
    Является списком для отображения карточек товаров в магазине. ДОбавляет товары в список.

    Конструктор:  
    `constructor(container: HTMLElement)` - принимает ссылку на DOM элемент, за отображение которого он отвечает. Заполняет поля класса.

    Поля класса:   
    `protected gallery: HTMLElement` - поле для хранения элемента отображающего список товаров в галлереи.  
