...
Методы:  
`productsList(): productsList: IProduct[]` - метод возвращает хранимый массив товаров.  
`productsList(productsList: IProduct[]): void` - метод сохраняет измененный массив товаров.  
`getProductByID(id: string):IProduct` - получение одного товара по его id.  
`focusCard(product: IProduct): void` - сохранение товара для подробного отображения в переменную focusCard.   
`focusCard(): IProduct` - получение товара для подробного отображения из переменной focusCard.  
...
Методы:  
`productsList(): IProduct[]` - получение массива товаров, который находится в корзине.  
...
Поля класса:  
`private payment: 'card' | 'cash' | ''` - способ оплаты выбранный пользователем.  
...
Методы:  
`payment(): string` - получить способ оплаты выбранный пользователем.  
`payment(val: 'card' | 'cash' | ''): void` - установить способ оплаты выбранный пользователем.  
`adress() string:` - получить адресс пользоватля.  
`adress(val: string): void` - установить адресс пользоватля.  
`phone(): string` - получить телефон пользоватля.  
`phone(val: string): void` - установить телефон пользоватля.  
`email(): string` - получить электронная почта пользоватля.  
`email(val: string): void` - установить электронная почта пользоватля.  
